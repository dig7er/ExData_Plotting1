...
# 2.2. Draw the graph
# 2.2.1. Define the layout for 4 sub-graphs
par(mfcol=c(2, 2))

# 2.2.2. Draw the 1st graph
with(myData, plot(Time, Global_active_power, type="l", xlab="", ylab="Global Active Power"))

# 2.2.3. Draw the 2nd graph
...
# 2.2.3.1. Add a legend to the 2nd graph
legend(
  "topright",
  ...
  bty = "n",
  y.intersp = 1.25,
  text.width = 0.5*48*60*60 # the length is set to 40% of the whole range 48 h
)

# 2.2.4. Draw the 3rd graph
with(myData, plot(Time, Voltage, type="l", xlab="datetime", ylab="Voltage"))

# 2.2.5. Draw the 4th graph
